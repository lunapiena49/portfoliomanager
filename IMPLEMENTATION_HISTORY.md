...
## Sessione 2026-04-27 -- Espansione catalogo provider market-data + alert quota

Trigger: richiesta utente di estendere il supporto API key oltre EODHD/FMP/Gemini, aggiungere selettore frequenza aggiornamento dati con spiegazione (snapshot pubblico vs chiavi personali) e alert utente quando un endpoint restituisce errore di quota / rate limit / chiave invalida.

- **Nuovo modello dati provider** (`lib/core/constants/market_data_providers.dart`): enum `MarketDataProvider` (10 valori) + `ProviderConfig` con id/displayName/baseUrl/signupUrl/docsUrl/keyParamName/keyParamStyle/refreshTier/exampleEndpoint/supportsHistoric/supportsRealtime/requiresKey + `ProviderCatalog` con i 10 provider verificati Apr 2026 (EODHD, FMP, Alpha Vantage, Twelve Data, Finnhub, Polygon.io, Marketstack, Tiingo, Nasdaq Data Link, Stooq). Endpoint, parametri auth e free quota tutti rivisti contro le docs ufficiali. Override storage key per EODHD/FMP per back-compat con installazioni esistenti.
- **Refresh interval enum** (`MarketDataRefreshInterval`): manual / hourly / every4h / every12h / daily con extension `.duration` e parser `fromStorage`.
- **Storage key + setting** (`AppConstants.marketDataRefreshIntervalKey`): chiave settings_box per il refresh interval scelto.
- **LocalStorageService esteso**: cache `_cachedProviderKeys` per provider non-EODHD/FMP, getter/setter generici `getProviderApiKey/setProviderApiKey`, `getMarketDataRefreshInterval/setMarketDataRefreshInterval`, `clearAllData()` esteso a tutte le secure-storage entry.
- **SettingsBloc + AppSettings esteso**: eventi `UpdateProviderApiKeyEvent(provider, key)` e `UpdateMarketDataRefreshIntervalEvent(interval)`; field `providerApiKeys: Map<MarketDataProvider, String>` e `marketDataRefreshInterval`; helper `hasProviderKey(provider)`; `hasMarketApiKey` esteso; resolver al boot da secure storage.
- **Servizio centralizzato alert quota** (`lib/services/api/api_quota_alert_service.dart`): singleton broadcast `Stream<ApiQuotaEvent>` con classifier `classify(statusCode, bodyHint)` che mappa 429 (rate-limit / quota-exhausted distinti per body keyword) e 401/403 (invalid-key / quota in 403). Helper top-level `reportProviderDioException(provider, dioException)`.
- **Integrazione classifier nei servizi esistenti**: `GeminiService._mapDioException` chiama il reporter; `EodhdService` e `FmpMarketService` routano tutte le chiamate Dio via `_guardedGet` che intercetta DioException, riporta al servizio quota e rilancia.
- **Listener globale UI** (`lib/core/widgets/api_quota_alert_listener.dart`): subscribe singolo allo stream, mostra `MaterialBanner` colorato con etichetta provider tradotta, body i18n, auto-dismiss 12s, de-duplicazione 60s su (provider, reason). Wired in `main.dart` con `scaffoldMessengerKey` condiviso col `MaterialApp.router`.
- **Refactor Settings UI** (`lib/features/settings/presentation/pages/settings_page.dart`): nuova sezione "Dati di mercato" con (1) intro snapshot vs chiavi, (2) tile "Frequenza aggiornamento" che apre dialog `RadioGroup<MarketDataRefreshInterval>` con descrizioni dettagliate, (3) lista espandibile dei 10 provider con chip free quota / real-time / historic, descrizione, endpoint di esempio copiabile, bottoni signup/docs, campo API key con save/clear/visibility-toggle (eccetto Stooq che e' `requiresKey: false`). Eventi `UpdateProviderApiKeyEvent` usati per save/clear; il bloc gestisce internamente il mirror su eodhdApiKey/fmpApiKey.
- **Nuove sezioni guida** (`lib/features/onboarding/presentation/pages/guide_page.dart`): aggiunti 3 ExpansionTile -- "Catalogo provider supportati", "Frequenza di aggiornamento dati", "Alert quota e rate limit". Contenuti dettagliati con tabelle di endpoint, formula `calls/day = positions x refresh_per_day`, esempi numerici per tier free.
- **i18n in tutti e 6 i locale** (`assets/translations/{it,en,es,fr,de,pt}.json`): chiavi `common.dismiss`; blocco `settings.market_data.*` con refresh_interval/options/descriptions; blocco `settings.providers.*` con sub-key per ogni provider; 3 sezioni guide; blocco top-level `api_quota.*`. Tutte le 6 lingue sincronizzate strutturalmente. ASCII-only mantenuto in DE/FR/ES/PT.
- **Outcome**: `flutter analyze` "No issues found!"; `flutter test` verde (34/34); 6 lingue allineate; nessuna chiave hardcoded; chiavi cifrate in `flutter_secure_storage`. Per i 7 provider nuovi (Alpha Vantage, Twelve Data, Finnhub, Polygon, Marketstack, Tiingo, Nasdaq Data Link) lo scope e' "salva chiave + endpoint documentato + classifier alert"; integrazione runtime fetch puo' essere abilitata per ogni provider in release future senza modifiche al modello dati o al SettingsBloc.
