...
class _AddPositionPageState extends State<AddPositionPage> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<PortfolioBloc, PortfolioState>(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('add_position.title'.tr()),
        ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: _isLoading
                        ...
                        : Text('add_position.add_button'.tr()),
                  ),
                ),
                SizedBox(height: 16.h),
              ],
  ...
  void _submitPosition() {
    ...
    if (portfolioState is! PortfolioLoaded) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('portfolio.no_portfolio_to_add'.tr()),
          backgroundColor: AppTheme.errorColor,
        ),
      );
      return;
    ...
    if (_costBasisController.text.isNotEmpty) {
      costBasis = double.parse(_costBasisController.text.replaceAll(',', '.'));
    }
    ...
    final position = Position(
      ...
      isin: _isinController.text.trim().isNotEmpty ? _isinController.text.trim().toUpperCase() : null,
      lastUpdated: DateTime.now(),
    );

    context.read<PortfolioBloc>().add(AddPositionEvent(position));
  }
}